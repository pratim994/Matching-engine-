...
namespace Trading {
    ...
    MarketOrderBook::MarketOrderBook(TickerId ticker_id, Logger *logger)
     : ticker_id_(ticker_id), orders_at_price_pool(ME_MAX_PRICE_LEVELS) , order_pool_(ME_MAX_ORDER_IDS) , logger_(logger) {
  ...
  auto MarketOrderBook::toString(boll detailed , bool validity_check) const -> std::string {
    ...
    auto printer = [&](std::stringstream &ss, MarketOrdersAtPrice *itr, Side side, Price &last_price,
                    ...
                    bool sanity_check){
                        ...
                        sprintf(buf, "<px:%3 n:%3s > %3-s @ %-5s(%-4s)"
                            ...
                            for(Auto o_itr = itr->first_mkt_order_;; o_itr = o_itr->next_order_){
                                ...
                                if(detailed){
                                    ...
                                    sprintf(buf, "[oid:%s q:%s p:%s n:%s]" ,
                                        ...
                                        orderIdString(o_itr->next_order_ ? o_itr->next_order_->order_id_ :OrderId_INVALID).c_str());
