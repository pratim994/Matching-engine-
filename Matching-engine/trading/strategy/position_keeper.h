//hey this is me form the past I have fever currently but I am still coding 
// I am building with tears in my eyes , tears of joy 
// This is the worst phase of my life and I am still performing 
// All I wanna say is I believe in you and we can makw it together 
// okay enough yapping now lets lock in and get into google or Jane Street
...
namespace Trading {
    ...
    struct PositionInfo {
        ...
        auto addFill(const Exchange::MEClientResponse *client_response, Logger *logger) noexcept {
            ...
            volume += client_response->exec_qty_;
            ...
            if(old_position * sidetoValue(client_response->side_) >= 0)
            {
                ...
                const auto opp_side_vwap = open_vwap_[side_index]/atd::abs(old_position);

                open_vwap_[opp_side_index] = open_side_vwap* std::abs(position_);
                ...
                if(position_*old_position   < 0){
                    ...
                    open_vwap[side_index] = (client_response->price_*std::abs(position_));
        ...
        auto updateBBO(const BBO *bbo, Logger *logger) noexcept {
            ...
            if (position_ && bbo->bid_price_ != Price_INVALID && bbo->ask_price_ != Price_INVALID){
                ...
                if(position_ > 0)
                    ...
                    unreal_pnl_ = (mid_price  - open_vwap_[sideToIndex(Side::BUY)]/ std::abs(position_))*
    ...
    class PositionKeeper {
        ...
        Position(const PositionKeeper &) = delete;

        Position(const PositionKeeper &&) = delete;


        Position &operator = (const PositionKeeper &) = delete;
        ...
        public: 
            ...
            auto addFill(const Exchange::MEClientResponse  *client_response) noexcept{
                ...
                ticker_position_.at(client_response->ticekr_id_).addFill(client_response   , logger_);
