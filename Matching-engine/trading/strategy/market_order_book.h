...
#include "exchange/market_data/market_data.h"
...
namespace Trading {
            ...
            auto updateBBO(bool update_bid, bool update_ask) noexcept {
                ...
                if(update_bid){
                    ...
                    if(bids_by_price_){
                      ...
                      for( auto order = bids_by_price_->first_mkt_order_->next_order_;  order != bids_by_price_->first_mkt_order_ ;
                ...
                if(update_ask){
                        ...
                        if(asks_by_price_){
                                ...
                                bbo_.ask_price_ = asks_by_price_->first_mkt_order_->qty_;

                                for(auto order = asks_by_price_->first_mkt_order_->next_order_; order != asks`->first_mkt_order_; order = order->next_order_)
    ...
    private:
        ...
        MarketUpdateType *asks_by_price_ = nullptr;
        ...
        private:
            ...
            auto addOrdersAtPrice(MarketOrdersAtPrice *new_orders_at_price) noexcept {
                ...
                price_orders_at_price_.at(priceTpIndex(new_orders_at_price->price_)) = new_orders_at_price;
          ...
          if ((new_orders_at_price->side_ == Side::BUY && new_orders_at_price->price_ > best_orders_by_price->price_) ||
            ...
            auto removeOrdersAtPrice(Side side, Price price) noexcept {
                ...
                if(UNLIKELY(orders_at_price->next_entry_ == orders_at_price)) {
                    (sid == Side::BUY ? bids_by_price_ : asks_by_price_) =  nullptr;
                ...
                price_orders_at_price_at(priceToIndex(price)) = nullptr;
            ...
            auto addOrder(MarketOrder *order) noexcept -> void {
                ...
                const auto orders_at_price = getOrdersAtPrice(orde->price_);
                ...
                if(!order_at_price){
                    ...
                    auto new_orders_at_price = orders_at_price_pool.allocate(order->side_,  order->price_, order, nullptr, nullptr);
                    ...
                    order->prev_order_ = first_order;

                    first_order->prev_order_ = older;
                }
