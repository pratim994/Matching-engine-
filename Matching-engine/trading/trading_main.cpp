...
int main(int argc , char **argv){
        ...
        if(argc < 3){
            ...
            FATAL("USAGE trading_main  CLIENT_ID ALGO_TYPE [CLIP_1 THRESH MAX_ORDER_SIZE_1 MAX_POS_1 MAX_LOSS_1][CLIP_2 THRESH_2 MAX_ORDER_SIZE_2 MAX_POS_2 MAX_LOSS_2]...");
        ...
        srand(client_id);
         ...
         Exchnage::CLientRequestLFQueue client_requests(ME_MAX_CLIENT_UPDATES);
         Exchnage::CLientResponseLFQueue client_responses(ME_MAX_CLIENT_UPDATES);
         ...
         for(int i = 3; i < argc; i+= 5, ++next_ticker_id){
            ...
            ticker_cfg.at(next_ticker_id) = { static_cast<Qty>(std::atoi(Argv[i])), std::atof(argv[i+1]),
                                            ...
                                            {static-cast<Qty>(std::atoi(argv[i+2])),
           ...
           logger->log("%:% %() % Starting Trade Engine...\n", __FILE__, __LINE__, __FUNCTION__, Common::getCurrentTimeStr(&time_str));
            ...
            if(algo_type == AlgoType::RANDOM)
            {
                ...
                std::vector<Exchnage::MEClientRequest> client_requests_vec;
                ...
                for(size_t i = 0; i < 100000; ++i) {
                    ...
                    auto cxl_request = client_requests_vec[cxl_index];price
