...
namespace Trading {
    ...
    MarketDataConsumer::MarketDataConsumer(Commom::ClientId client_id, Exchange::MEMarketUpdateLFQueue *market_updates,
            ...
            iface_(iface) , snapshot_ip_(snapshot_ip), snapshot_port_(snapshot_port) {
                ...
                ASSERT(incremental_mcast_socket_.init(incremental_ip, iface , incremental_port, true) >= 0,
                    ...
                    "Unable to create incremental mcast socket. error. error:" +  std::string(std::strerror(errno)));
        ...
        auto MarketDataConsumer::run() noexcept -> void {
         ...
         auto MarketDataConsumer::startSnapshotSync()-> void {
            ...
            ASSERT(snapshot_mcast_socket_.init(snapshot_ip_, iface_, snapshot_port_, true) >= 0,
                    ...
                    "unable to ccreate snapshot mcast socket .error:" + std::string(std::sterror(errno)));
         ...
         auto MarketDataConsumer::queuedMessage(bool is_snapshot, const Exchange::MDPMarketUpdate *request) {
        ...
        auto MarketDataConsumer::recvCallback(McastSocket *socket) noexcept -> void {
            ...
            if(socket->next_rcv_valid_index >= sizeof(Exchange::MDPMarketUpdate)) {

                size_t i =0;
                for(; i +  sizeof(Exchange::MDPMarketUpdate) <= socket->next_rcv_valid_index; i += sizeof(Exchange::MDPMarketUpdate))
                {
                    ...
                    logger_.log("%:% %() % Received % socket len:% %\n", __FILE__, __LINE__, __FUNCTION__,
                        ...
                        queuedMessage(is_snapshot, request);
