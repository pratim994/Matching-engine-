     ...
     class Logger final {
        public:
         ...
         explicit Logger(const std::string &file_name) : 
            file_name(file_name), queue_(LOG_QUEUE_SIZE){
                file_.open(file_name);

                ASSERT(file_.is_open(), "COULD NOT OPEN THE LOG FILE" + file_name);
               
                logger_thread_ = createAndStartThread(-1, "commomn/logger " + file_name_ , [this]() {flushQueue(); });

                ASSERT(logger_thread_ != nullptr, " failed to start a logger thread.");
