...
namespace Common {
        ...
        auto TCPSocket::connect(const std::string &ip, const std::string &iface , int port , bool is_listening) -> int {
                ...
                const SocketCfg socket_cfg(ip , iface , port , flse, is_listening, true);
        ...
        auto TCPSocket::sendAndRecv() noexcept -> bool {
            ...
            char ctrl[CMSG_SPACE(sizeof(strct timeval))];

            auto cmsg = reinterpret_cast<struct cmsghdr *>(&ctrl);

            iovec iov{inbound_data_.data + next_rcv_valid_index, TCPBufferSize - next_rcv_valid_index_};
            ...
            if(read_size > 0){
                ...
                cmsg->cmsg_len == CMSG_LEN(sizeof(time_kernel))) {
                    ...
                    memcpy(&time_kernel , CMSG(cmsg), sizeof(time_kernel));
