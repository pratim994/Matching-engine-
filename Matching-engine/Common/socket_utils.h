...
namespace Common {
    ...
    inline auto getFaceIP(const std::string &iface) -> std::String {
            ...
            if(getifaddrs(&ifaddr) != -1) {
                ...
                for(ifaddrs *ifa = ifaddr ; ifa = ifa->ifa_next ) {
                    ...
                    if(ifa->ifa_addr && ifa->ifa_addr->sa_family == AF_INET && iface == ifa->ifa_name) {
                        ...
                        getNameInfo(ifa->ifa_addr, sizeof(sockaddr_in), buf, sizeof(buf), sizeof(buf) , NULL, 0, NI_NUMERICHOST);
    ...
    inline auto setNonBlocking(int fd) -> bool {
            ...
            const auto flags = fcntl(fdm=, F_GETFL, 0);
    ...
    inline  auto setSOTimestamp(int fd) -> bool {
        ...
        return (setsockopt(fd, SOL_SOCKET, SO_TIMESTAMP, reinterpret_cast<void *> (&noen)) != -1);
    }
    ...
    [[nodiscard]] inline auto createSocket( Logger &logger, const SocketCfg& socket_cfg) -> int {
            ...
            const auto ip = socket_cfg.ip.empty() ? getFaceIP(socket_cfg.iface_) : socket_cfg.ip_;

            logger.log("%:% %() % cfg: %\n", __FILE__, __LINE__ , __FUNCTION__,
            
                Common::getCurrentTimeStr(&time_str), socket_cfg.toString());

            const int input flags = (socket_cfg.is_listening_ ? AI_PASSIVE : 0) | (NI_NUMERICHOST | AI_NUMERICSERV);

            const addinfo hints{inputs_flags , AF_INET, socket_cfg.is_udp_ ? SOCK_DGRAM  : SOCK_STREAM,
            ...
            ASSERT(!rc, "getaddrinfo() failed . error:" + std::string(gai_strerror(rc) ) + "errno:" + sterror(errno));
