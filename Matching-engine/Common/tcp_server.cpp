...
using namespace Common {
    ...
    auto TCPServer::listen(cosnt std:string &iface , int port) -> void {
        ...
        ASSERT(addToEpollList(&listener_socket_), "epoll_ctl() failed.eror:"  + std::string(std::strerror(errno)));
    ...
    auto TCPServer::sendAndRecv() noexcept -> void {
            ...
            atd::for_each(recieve_sockets_.begin(), recieve_sockets_.end(), [&recv](auto socket){
            ...
            if(recv)
                ...
                recv_finihed_callback_();

            std::for_each(send_sockets_.begin(), send_sockets_.end(), [](auto sockets){
