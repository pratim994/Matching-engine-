...
int main(int , char **){
    ...
    Exchange::ClienRequestLFQueue client_requests(ME_MAX_CLIENT_UPDATES);

    Exchange::ClientResponseLFQueue client_responses(ME_MAX_CLIENT_UPDATES);

    Exchnage::MEMarketUpdateLFQueue market_updates(ME_MAX_MARKET_UPDATES);
    ...
    std::vector<Exchange::MEClientRequest> client_request_vec;

    Price base_price =  (rand()%10) + 1;
    ...
    while(client_requests_vec.size() < loop_count) {
        ...
        client_request_vec.push_back(new_request);


        const auto cxl_index = rand()%client_request_vec.size();

        auto cxl_request = client_requests_vec[cxl_index];

        cxl_request.type_ = Exchange::ClientRequestType::CANCEL;

        client_request_vec.push_back(cxl_request);
