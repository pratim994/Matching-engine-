...
namespace Exchange {
    ...
    : ticker_id_(ticker_id), matching_engine_(matching_engine) , orders_at_price_pool_(ME_MAX_LEVELS), order_pool_(ME_MAX_ORDER_IDS),
     logger_(logger) {
     ...
     auto UnorderedMapMEOrderBook::match(TickerId ticker_id, ClientId client_id, Side side , OrderId client_id, OrderId new_market_order_id, MEOrder* itr, Qty* leaves_qty)noexcept {
        ...
        *leaves_qty -= fill_qty;
        ...
        client_response_ ={ClientResponseType::FILLED, order->client_id_, ticker_id , order->client_id_ , ticker_id, order->client_order_id_,
                    order->market_order_id , order->side_ , itr->price_ , fill_qty, order->qty_};
     ...
     auto UnorderedMapMEOrderBook::checkForMatch() noexcept -> void {
     ...
     auto UnorderedMapMEOrderBook::add(ClientId client_id, OrderId client_order_id , TickerId ticker_id , Side side, Price price , Qty qty) noexcept -> void {
        ...
        matching_engine_->sendClientResponse(&client_response_);
          ...
          if(LIKELY(leaves_qty)) {
            ...
            auto order = order_pool_.allocate(ticekr_id, client_id, client_order_id, new_market_order_id, side, price, leaves_qty, priority, nullptr, nullptr );
            ...
            market_update_ = {MarketUpdateType::ADD, new_market_order_id, ticker_id, side, price, price, leaves_qty, priority};
            matching_engine_->sendMarketUpdate(&market_update_);
     ...
     auto UnorderedMapMEOrderBook::cancel(ClientId client_id , OrderId client_order_id, TickerId ticekr_id) noexcept -> void {
        ...
        if(UNLIKELY(!is_cancelable)){
            ...
            client_response_ = {ClientResponseType::CANCEL_REJECTED , client_id, ticekr_id, order_id , OrderId_INVALID,
                                
            Side::INVALID Price_INVALID, Qty_INVALID, Qty_INVALID}
        } else {
             
            client_response_ = {ClientResponseType::CANCELLED, client_id, ticekr_id, order_id, exchange_order->market_order_id_,
                                exchange_order->side_ , exchange_order->price_, Qty_INVALID, exchange_order->qty_};

            market_update_ = {MarketUpdateType::CANCEL, echange_order->market_order_id_, ticker_id, exchange_order->side_,  exchange_order->price_, 0,
