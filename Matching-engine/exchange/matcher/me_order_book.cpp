...
MEOrder::MEOrderBook(TickerId ticker_id, Logger *logger,
    MatchingEngine *matching_engine
        ...
        MEOrderBook:: ~MEOrderBook() {
            ...
            auto MEOrder::match(TickerId ticker_id, ClientId client_id, Side side, OrderId client_order_id, OrderId new_market_order_id, MEOrder* itr, Qty* leaves_qty ) 
                noexcept {
                    ...
                    *leaves_qty -= fill_qty;
                    ...
                    client_response_ = {CLientResponseType::FILED, client_id , ticker_id, client_order_id,
                            new_market_order_id, side , itr->price_, fill_qty, *leaves_qty};

                            matching_engine_ -> sendClient(&client_response_);

                            client_response_= {CLientResponseType::FILLED, order->client_id_, ticker_id, order->client_order_id,
                            
                                    order->market_order_id_, order->side_, itr->side_, fill_qty, order->qty_};
                            ...
                            if(!order->qty_){
                                market_update_ = {MarketUpdateType::CANCEL, order->market_order_id, ticker_id, order->side_,
        ...
        auto MEOrderBook::checkForMatch(Client client_id, OrderId client_order_id, Ticker_id ticker_id, Side side, Price price, Qty qty , Qty new_market_order_id) noexcept {
            auto leaves_qty = qty;
            ...
            if(side == Side::BUY){
                while(leaves_qty && asks_by_price_) {
                    ...
                    if(LIKELY(priced < ask_itr->price_)){
                        break;
                    }
            ...
            if(side == Side::SELL){
                while(leaves_qty && bids_by_price_){
                    ...
                    if(LIKELY(price > bids_by_price_)) {
                        break;
                    }

                    match(ticker_id, client_id, client_order_id, new_market_order_id, ask_itr , &leaves_qty);
                 }
            }

            if(side == side::SELL) {
                while(leaves_qty & bids_by_price_) {
                    cosnt auto bid_itr = bids_by_price_->first_me_order_;
        ...
        auto MEOrderBook::add(Client client_id, OrderId client_order_id, TickerId ticker_id,Side side , Price price, Qty qty)
                noexcept -> void {
                    //concept  well  I  accidentally discovered a new keyword of c++ 20 XO

                    const auto new_market_order_id = generateNewMarketOrderId();

                    client_response_ = {CLientResponseType::ACCEPTED , client_id, ticker_id , client_order_id, new_market_order_id, side , price ,0
                            qty};
                            ...
                            if(LIKELY(leaves_qty)) {
                                ...
                                auto order = order_pool.allocate(ticker_id, client_order_id, new_market_order_id, side , price , leaves_qty, priority, nullptr,
                                nullptr);
                ...
                auto  MEOrderBook::cancel(ClientId client_id, OrderId ordder_id, TickerId ticker_id,) noexcept -> void {
                        ...
                        if(UNLIKELY(!is_cancelable)){
                            client_response_ = {ClientResponseType::CANCEL_REJECTED, client_id, ticker_id, order_id, OrderId_INVALID,
                                            Side::INVA:LID, Price_INVALID, Qty_INVALID, Qty_INVALID};


                        } else {
                            client_response_ =  {ClientResponseType::CANCELED, client_id, ticker_id, order_id, exchange_order->market_order_id, 
                                            exchange_order->side_, exchange_order->price_, Qty_INVALID, exchange_order->qty_};

                            market_update_ = {MarketUpdateType::CANCEL, exchange_order->market_order_id_, ticker_id, exchange_order->side_, 
                    ...
                    auto printer  = [&](std::stringstream &ss, MEOrderAtPrice *itr, Side side, Price &last_price, bool sanity_check)
                        {
                            ...
                            for(auto o_itr = itr->first_me_order_;; o_itr->next_order_) {
                                qty += o_itr->qty_;
