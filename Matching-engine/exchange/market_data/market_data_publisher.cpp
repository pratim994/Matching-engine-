...
namespace Exchange {
        ...
        MarketDataPublisher::MarketDataPublisher(MEMarkedtUpdateLFQueue *market_updates, const std::string &iface,
        
                        const std::string &snapshot_ip, int snapshot_port,
                    
                        const std::string &incremental_ip, int increment_port) :

        outgoing_md_updates_(market_updates), snapshot_md_updates_(ME_MAX_UPDATES),
            
        run_(false) , logger_("exchange_market_data_pubisher.log") , incremental_socket_(logger_) {
                ...
                snapshot_synthesizer_ = new SnapShotSynthesizer(&snapshot_md_updates_ , iface, snapshot_ip, snapshort_port);
                    ...
                    for(auto market_update = outgoing_md_updates_->getNextToRead();
                        ...
                        market_update = outgoing_md_updates_ ->getNextToRead()
                            ...
                            logger_.log("%:% %() % Sending seq:% %\n", __FILE__, __LINE__, 
                                __FUNCTION__, Common::getCurrentTimeStr(&time_str_), next_inc_seq_num_,
