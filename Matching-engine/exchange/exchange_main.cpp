...
int main(int , char **)
{
    ...
    std::signal(SIGNINT, signal_handler);

    const int sleep_time = 10*1000;

    Exchange::ClientRequestLFQueue
        client_request(ME_MAX_CLIENT_UPDATES);
    ...
    Exchange::MEMarketUpdateLFQueue
        ...
        logger->log("%:% %() % Starting Matching Engine..\n",
            ...
            matching_engine =  new
            Exchange::MatchingEngine(&client_requests, &client_responses,
                    &market_updates);
        ...
        market_data_publisher = new Exchange::MarketDataPublisher(&market_updates, mkt_pub_iface, snap_pub_ip,snap_pub_port,
